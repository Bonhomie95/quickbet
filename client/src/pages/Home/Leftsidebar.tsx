...
const Leftsidebar = () => {
  ...
  return (
    <div className="hidden md:flex flex-col items-left justify-left gap-2">
      ...
      <div className="flex flex-col gap-2 items-center justify-center">
        ...
        <ul className="flex flex-col gap-3 items-start justify-center text-left w-full">
          {sports.map((sport) => (
            <li
              key={sport.id}
              className={`px-12 text-[#222] opacity-40
              ${active === null && sport.id === 'football' && 'opacity-100'}
              ${sport.id === active && 'opacity-100'}`}
            >
              <Link  key={sport.id} to={sport.id} onClick={() => addActive(sport.id)}>
                {' '}
                {sport.title}{' '}
              </Link>
            </li>
